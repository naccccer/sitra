...
import { Button, WorkspaceCard } from '@/components/shared/ui';
    ...
    <div data-layer-key={layerKey} className={`relative mx-1 flex flex-col overflow-hidden rounded-xl border shadow-sm sm:h-11 sm:flex-row ${isUnavailable ? 'border-red-300 bg-red-50/40' : 'border-slate-200 bg-white'}`}>
      ...
      <div className={`flex flex-1 flex-wrap items-center gap-1.5 p-1 ${isUnavailable ? 'bg-red-50/20' : 'bg-white'}`}>
        <select value={data.glassId} onChange={(event) => onChange('glassId', event.target.value)} className={`h-8 min-w-0 grow basis-[130px] rounded-lg border bg-slate-50 px-2 py-1.5 text-[11px] font-black outline-none ${isUnavailable ? 'border-red-300 text-red-700' : 'border-slate-200'}`}>
          {!selectedExists && selectedGlass ? <option value={selectedGlass.id}>{selectedGlass.title} (ناموجود)</option> : null}
...
export const OrderConfigurationSection = ({
  ...
  return (
    <WorkspaceCard
      ...
      title="پیکربندی سفارش"
      description="انتخاب ساختار، ابعاد، خدمات و قیمت‌گذاری هر آیتم قبل از افزودن به سفارش."
      padding="none"
      bodyClassName="!p-0"
    >
      <div className="flex flex-wrap gap-2 border-b border-[rgba(var(--ui-border),0.9)] bg-[rgba(var(--ui-surface-muted),0.72)] p-3 sm:gap-3">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`min-w-[95px] flex-1 rounded-[var(--radius-lg)] border py-3.5 text-sm font-black transition-[background-color,border-color,color,box-shadow] ${
              activeTab === tab.id
                ? 'border-[rgba(var(--ui-primary),0.24)] bg-[linear-gradient(135deg,rgba(var(--ui-primary),0.92),rgba(var(--ui-primary-soft),0.92))] text-[rgb(var(--ui-primary-contrast))] shadow-ui-soft'
                : 'border-[rgba(var(--ui-border),0.82)] bg-[rgba(var(--ui-surface-elevated),0.9)] text-[rgb(var(--ui-text-muted))] hover:text-[rgb(var(--ui-text))]'
            }`}
          >
            {tab.label}
          </button>
      ...
      <div className="flex flex-col items-start gap-8 p-6 lg:flex-row">
        <div className="w-full flex-1">
          {activeTab === 'single' ? <div className="mx-auto max-w-2xl"><GlassRow layerKey="single.glass1" isUnavailable={Boolean(unavailableLayers['single.glass1'])} data={config.single} onChange={(field, value) => updateConfigLayer('single', field, null, value)} catalog={catalog} /></div> : null}
          {activeTab === 'laminate' ? (
          ...
          {activeTab === 'custom' ? (
            <div className="mx-auto w-full max-w-2xl space-y-3 rounded-2xl border border-[rgba(var(--ui-border),0.85)] bg-[rgba(var(--ui-surface-muted),0.78)] p-4 shadow-[var(--shadow-inset)]">
              <div className="flex items-center justify-between gap-2">
                <div className="text-xs font-black text-[rgb(var(--ui-text))]">انتخاب آیتم سفارشی</div>
                <Button type="button" onClick={onGoToCustomItems} variant="secondary" size="sm">
                  <Plus size={12} />
                  ساخت آیتم
                </Button>
              </div>
        ...
        <div className="surface-card-quiet flex w-full flex-col gap-4 rounded-[var(--radius-xl)] p-5 lg:w-[320px]">
          ...
          <Button onClick={onOpenSettingsModal} disabled={isCustomServiceDisabled} variant="secondary" className="w-full py-3.5">
            <Settings size={16} />
            <span className="text-xs font-black">خدمات و الگو</span>
            {!isCustomServiceDisabled && (Object.keys(config.operations || {}).length > 0 || config.pattern?.type !== 'none') ? <span className="mr-2 rounded-full bg-[rgb(var(--ui-warning))] px-1.5 text-[9px] text-white">ثبت شد</span> : null}
          </Button>
          {isCustomServiceDisabled ? <div className="text-[10px] font-bold text-[rgb(var(--ui-text-muted))]">برای این واحد، خدمات و الگو غیرفعال است.</div> : null}
          ...
          <button onClick={onAddCatalogItem} disabled={!canAddCatalogItem} className={`mt-1 flex w-full justify-between rounded-[var(--radius-xl)] p-1.5 text-sm font-black text-white shadow-ui-soft transition-all ${!canAddCatalogItem ? 'cursor-not-allowed bg-[rgba(var(--ui-text-muted),0.45)]' : 'bg-[linear-gradient(135deg,rgba(var(--ui-primary),0.96),rgba(var(--ui-primary-soft),0.92))] hover:brightness-105 active:scale-95'}`}>
