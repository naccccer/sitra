...
export const CustomerDetailsModal = ({
  ...
  return (
    <ModalShell
      ...
      bodyClassName="max-h-[82vh] space-y-4 bg-slate-50 p-5"
      footerClassName="bg-slate-50 px-5 py-4"
      footer={(
        <div className="flex items-center justify-between gap-3">
          <div className={`min-h-5 text-xs font-black ${error ? 'text-rose-700' : 'text-slate-500'}`}>
            {error || ' '}
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white p-2">
        {DETAILS_TABS.map((tab) => (
          <button
            ...
            className={`rounded-xl px-3 py-2 text-xs font-black transition-colors ${activeTab === tab.id ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
            {tab.label}
          </button>
        ))}
