...
export const LoginView = ({ onLogin, onGoToCustomer }) => {
  ...
  const [isLoading, setIsLoading] = useState(false); // برای حالت در حال بارگذاری
  const [errorMsg, setErrorMsg] = useState(''); // برای نمایش خطای سرور
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // ⚠️ آدرس زیر رو باید تو محیط اصلی با دامنه خودت عوض کنی 
      // مثلا: https://yourdomain.com/api/login.php
      const response = await fetch('http://localhost/api/login.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        credentials: 'include', // <--- جواب سوالت: اینجاست! کوکی‌ها رو می‌فرسته
      ...
      if (response.ok && data.success) {
        // لاگین موفق بود، نقش کاربر رو از دیتابیس می‌گیریم
        onLogin(data.role); 
      } else {
        // سرور ارور داده (پسورد غلط، یوزر اشتباه و ...)
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans" dir="rtl" style={{ fontFamily: 'Vazirmatn' }}>
       <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm border border-slate-100 animate-in fade-in zoom-in-95">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
             ...
             {/* نمایش پیام خطا به جای alert رو اعصاب */}
             {errorMsg && (
             ...
             <button disabled={isLoading} type="submit" className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-black py-3.5 rounded-xl flex justify-center items-center gap-2 shadow-lg shadow-blue-600/30 transition-all mt-6">
