...
export const PaginationBar = ({
  ...
  <div className={cn('flex flex-col gap-1.5 rounded-b-[var(--radius-xl)] px-2 py-1 sm:flex-row sm:items-center sm:justify-between', className)}>
    ...
    <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap">
      {onPageSizeChange && pageSizeOptions.length > 0 ? (
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          ...
          <Select className="min-w-[80px] whitespace-nowrap bg-transparent shadow-none" size="sm" value={String(pageSize)} onChange={(event) => onPageSizeChange(Number(event.target.value))}>
            {pageSizeOptions.map((option) => <option key={option} value={option}>{option} ردیف</option>)}
          </Select>
        </div>
      ) : null}
