        ...
        <div className="bg-white w-full max-w-3xl rounded-2xl flex flex-col max-h-[85vh] shadow-2xl animate-in zoom-in-95">
          ...
          <div className="p-3 overflow-y-auto flex-1 bg-slate-50/50 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {catalog.operations.map(service => (
              <div key={service.id} onClick={() => toggleService(service.id)} className={`cursor-pointer bg-white border-2 rounded-xl overflow-hidden flex flex-col aspect-[2/3] max-w-[150px] w-full justify-self-center transition-all ${selectedServices[service.id] ? 'border-blue-500 bg-blue-50/30 shadow-md' : 'border-slate-200 hover:border-slate-300'}`}>
                ...
                <div className="min-h-12 px-2 py-2 text-center text-[10px] leading-4 font-bold border-t border-slate-100 text-slate-700 flex items-center justify-center">{service.title}</div>
              </div>
            ))}
          </div>
