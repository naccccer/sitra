...
export const AdminUsersSettingsTab = ({ session }) => {
  ...
  const createValidation = useMemo(() => {
    ...
    if (!roleOptions.some((option) => option.value === role)) return 'سطح دسترسی نامعتبر است.';
    return '';
  ...
  const toggleActive = async (user) => {
    ...
    setSuccessMsg('');
    try {
      const response = await api.setUserActive(Number(user.id), nextActive);
      if (response?.user) {
        setUsers((prev) => prev.map((candidate) => (String(candidate.id) === String(user.id) ? response.user : candidate)).sort(userSort));
      } else {
        await loadUsers();
                    ...
                    <tr key={user.id} className={user.isActive ? '' : 'bg-slate-50/80'}>
                      ...
                      <td className="p-2">
                        {isEditing ? (
                          ...
                          <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-black ${user.role === 'admin' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-700'}`}>
                            {user.role === 'admin' ? 'ادمین' : 'مدیر'}
                          </span>
                        )}
                      </td>
                      <td className="p-2">
