...
| Human Resources Employee Directory | `src/modules/human-resources` | `api/modules/human_resources` | `hr_employees` | `GET|POST|PUT|PATCH /api/hr_employees.php` |
| Accounting Payroll Management | `src/modules/accounting` | `api/modules/accounting` | `acc_payroll_periods`, `hr_employees`, `acc_payslips`, `acc_payslip_items`, `acc_payslip_payments`, `acc_payslip_documents`, `acc_vouchers`, `acc_voucher_lines`, `acc_accounts`, `system_settings` | `GET|POST|PUT|PATCH /api/acc_payroll.php`, `GET|POST /api/acc_settings.php?key=accounting.payroll.settings`, `POST /api/acc_payroll_import.php` |
