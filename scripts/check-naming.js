...
const ALLOWED_SNAKE_NAMESPACES = new Set([
  'auth',
  'users_access',
  'master_data',
  'sales',
...
function checkAuditEventTypes(filePath, source) {
  ...
  while ((match = regex.exec(source)) !== null) {
    ...
    if (
      !/^(auth|users_access|master_data|sales|customers|inventory|kernel|accounting|accounting_payroll|human_resources)\.[a-z_]+(\.[a-z_]+)+$/.test(
        eventType,
      )
    ) {
